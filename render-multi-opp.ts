...
const d = JSON.parse(readFileSync("/tmp/multi/top_opp.json", "utf8"));
const top: any[] = d.top;
...
function rows(items: any[], startIdx: number) {
  return items
    .map((r, i) => {
      ...
      const moreLikely = ((r.invRatio - 1) * 100).toFixed(1);
      ...
      return `<tr>
        ...
        <td class="num ratio">${r.invRatio.toFixed(3)}×</td>
        <td class="num edge">+${moreLikely}%</td>
      </tr>`;
    })
    .join("");
}
...
function page(items: any[], startIdx: number, partLabel: string) {
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .sprd{color:#8b949e;font-size:12.5px}
  ...
  <h1>World Cup 2026 — Polymarket prices LONGER than the market (Shin) · ${partLabel}</h1>
  <div class="sub">Top 40 outcomes where Polymarket rates a bet <i>less</i> likely than the de-vigged ~40-book consensus — sorted by how much more probability the market gives it than Polymarket.</div>
  <div class="leg"><b>PM</b> = Polymarket decimal odds · <b>Spread</b> = Polymarket order-book bid/ask spread (cents) · <b>NT</b> = Norsk Tipping raw · <b>NT-Shin</b> = NT de-vigged (Shin) · <b>OA-Shin</b> = Odds-API average-of-all-books, de-vigged (Shin) · <b>Ratio</b> = OA-Shin % ÷ PM-implied % · <b>Mkt+</b> = how much more likely the market rates it vs. Polymarket.</div>
  <table>
  <thead><tr>
    <th class="rank">#</th><th>Match</th><th>Market</th><th>Selection</th>
    <th class="num pmh">PM</th><th class="num">Spread</th><th class="num">NT</th><th class="num">NT-Shin</th>
    <th class="num oah">OA-Shin</th><th class="num">Ratio</th><th class="num">Mkt+</th>
  </tr></thead>
...
(async () => {
  ...
  const parts = [
    { items: top.slice(0, 20), start: 0, label: "Part 1 (1–20)", file: "/tmp/multi/top40_opp_p1.png" },
    { items: top.slice(20, 40), start: 20, label: "Part 2 (21–40)", file: "/tmp/multi/top40_opp_p2.png" },
  ];
